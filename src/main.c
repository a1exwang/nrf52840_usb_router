...
static int bdb_start(int role)
{
	...
	if ((!zigbee_is_stack_started()))
	{
		...
		case ZB_NWK_DEVICE_TYPE_ED:
			...
			LOG_INF("Started End Device");
			break;
		default:
			LOG_ERR("Role unsupported");
			return -ENOEXEC;
		}
	...
	if (ret)
	{
		LOG_INF("bdb started successfully");
		return 0;
	}
	else
	{
		LOG_ERR("Could not start top level commissioning");
		return -ENOEXEC;
	}
}
...
void zboss_signal_handler(zb_bufid_t bufid)
{
	...
	LOG_INF("zboss_signal_handler %d", sig);
...
void main(void)
{
	...
	// LOG_INF("Device STARTED OK, start commissioning");
	// zb_bdb_set_legacy_device_support(1);
	// bdb_start_top_level_commissioning(ZB_BDB_NETWORK_STEERING);
	bdb_start(ZB_NWK_DEVICE_TYPE_ROUTER);
}
